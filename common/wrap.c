...
{
#if CZ_POSIX_VERSION <= CZ_POSIX_1990
	errno = 0;
#endif
...
enum CzResult czWrap_mmap(void* restrict* res, void* addr, size_t size, int prot, int flags, int fd, off_t offset)
{
	...
	case EINVAL:
		...
		if (offset < 0)
			return CZ_RESULT_BAD_OFFSET;
		if (flags & (MAP_ANON | MAP_GUARD) && offset)
			return CZ_RESULT_BAD_OFFSET;
		if (flags & (MAP_ANON | MAP_GUARD) && fd != -1)
			return CZ_RESULT_BAD_ACCESS;
		...
		if (flags & MAP_GUARD && prot != PROT_NONE)
			return CZ_RESULT_BAD_ACCESS;
		if (flags & MAP_GUARD && flags & (MAP_ANON | MAP_PREFAULT | MAP_PREFAULT_READ))
			return CZ_RESULT_BAD_ACCESS;
		if (flags & MAP_GUARD && flags & (MAP_PRIVATE | MAP_SHARED | MAP_STACK))
			return CZ_RESULT_BAD_ACCESS;
		...
		long pageSize = sysconf(_SC_PAGESIZE);
		if (flags & MAP_FIXED && pageSize > 0 && (uintptr_t) addr & (uintptr_t) (pageSize - 1))
			return CZ_RESULT_BAD_ALIGNMENT;
		if (flags & MAP_FIXED)
			return CZ_RESULT_BAD_ADDRESS;
		return CZ_RESULT_INTERNAL_ERROR;
...
{
	BOOL r = GetFileInformationByHandleEx(file, fileInformationClass, fileInformation, bufferSize);
	if CZ_EXPECT (r)
		return CZ_RESULT_SUCCESS;
	...
	case ERROR_HANDLE_EOF:
		if (fileInformationClass == FileStreamInfo) {
			if (bufferSize >= sizeof(DWORD))
				((PFILE_STREAM_INFO) fileInformation)->NextEntryOffset = 0;
			return CZ_RESULT_SUCCESS;
		}
		return CZ_RESULT_INTERNAL_ERROR;
	case ERROR_ACCESS_DENIED:
...
enum CzResult czWrap_SetFileInformationByHandle(
	HANDLE file, FILE_INFO_BY_HANDLE_CLASS fileInformationClass, PVOID fileInformation, DWORD bufferSize)
{
	BOOL r = SetFileInformationByHandle(file, fileInformationClass, fileInformation, bufferSize);
	if CZ_EXPECT (r)
		return CZ_RESULT_SUCCESS;
...
{
	DWORD r = GetFileType(file);
	DWORD err = GetLastError();
	if CZ_EXPECT (r || err == ERROR_SUCCESS) {
		*res = r;
		return CZ_RESULT_SUCCESS;
	}
